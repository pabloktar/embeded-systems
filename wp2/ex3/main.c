...
int main(void){
    ...
    while (exitProgram != 1) { // While the user has not chosen to exit the program, continue.
        ...
        if (result < 0 || (selectMenuOption > 5) || (selectMenuOption < 1)) { // Check to ensure that the user follows input restrictions.
...
void search_by_firstname(char *name) {
    ...
    if (!personFound) { // Checks if the person was not found
